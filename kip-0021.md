...
6. Expose a per-block structural back-link (`inactivity_shortcut(B)`) so L2 guest verifiers can prove lane inactivity in `O(N/F)` walked anchors without knowing the value of `F`.
...
* Proving/witness implementers: read *Per-Lane Block Activity Digest (§4.2)*, *Update or Initialize Lane Tips (§5.2)*, and *Commitment Structure (§6)* (especially §6.3–§6.8), then the standalone *Inactivity Shortcut Spec* (referenced from §6.6), then *Complexity (§10)* and *Optional Persistent Witness Store (§11)*.
...
7. wrap `ActiveLanesRoot(B)` with `inactivity_shortcut(B)` to obtain `ActivityRoot(B)`;
8. compute block context hash (`timestamp`, `daa_score`, `blue_score`) and miner-payload root;
9. combine these into `SeqStateRoot(B)`;
10. chain through selected parent to get `SeqCommit(B)`;
11. publish `SeqCommit(B)` in `accepted_id_merkle_root`.
...
* Activity Root (`ActivityRoot(B)`): per-block wrap of `ActiveLanesRoot(B)` together with the `inactivity_shortcut(B)` field (Section 6.6).
* Sequencing State Root (`SeqStateRoot(B)`): commits to `ActivityRoot(B)` and additional context/miner-payload data at chain block `B` (Section 6.7). Distinct from the UTXO commitment (`utxo_commitment`).
* Sequencing Commitment (`SeqCommit(B)`): the value stored in `B.header.accepted_id_merkle_root` (Section 6.8).
...
Consensus rule change: transactions may carry **any non-reserved 20-byte `subnetwork_id`**. This is a deliberate relaxation of the prior consensus rule that constrained user transactions to the native subnetwork ID, and it is what enables user-chosen lane addressing for based-rollup-style L2 routing.
...
| `H_collapsed_node` | `"SeqCommitActiveCollapsedNode"` |
| `H_seq` | `"SeqCommitmentMerkleBranchHash"` |

`H_activity_root` and `H_collapsed_node` are introduced by this revision; see Section 6.6 (activity-root wrap) and Section 6.4.1 (SLO collapsed-subtree optimization) respectively.
...
```
tx_digest(tx) = tx.id || le_u16(tx.version)
```

where `tx.id` is the 32-byte transaction ID and `tx.version` is the 2-byte little-endian transaction version. Version is committed inline so an upper layer can distinguish transactions that share an ID across version boundaries.
...
* Single entry root is the entry itself (standard Merkle convention, matching `kaspa_merkle::calc_merkle_root`). Domain-separated leaf hashers keep the leaf-vs-internal preimage spaces disjoint, so this collapse is safe.
* Internal nodes are `H_seq(left, right)`. Missing right siblings at the leaf level pad with `ZERO_HASH`.
...
```
TipUpdateHash(parent_ref, lane_key(lane), activity_digest, MergesetContextHash(B)) =
    H_lane_tip(parent_ref || lane_key(lane) || activity_digest || MergesetContextHash(B))
```

where:

* `parent_ref` is either the previous lane tip hash (if lane is already active) or a global anchor (Section 5.2).
* `lane_key(lane) = H_lane_key(lane_id_bytes(lane))` is the 32-byte hashed lane key (Section 6.1). Using the hashed key here (rather than raw `lane_id_bytes`) keeps the preimage a fixed width and lets the lane tip be reconstructed by anyone holding `lane_key` directly, without recomputing the hash from `lane_id_bytes`.
...
4. Compute `tip_next = TipUpdateHash(parent_ref, lane_key(lane), activity_digest_lane(B), ctx_hash_B)`.
5. Set:
...
Let `F` (the *finality-depth* parameter) be a protocol constant measured in blue score units.

A lane `lane` is **present** in `ActiveLanes` from `B`'s POV iff:

```
ActiveLanes[lane].last_touch_blue_score + F >= B.blue_score
```

equivalently, lanes are purged at block `B` iff:

```
ActiveLanes[lane].last_touch_blue_score + F < B.blue_score
```

(strict inequality). Purged lanes are removed from `ActiveLanes`. The boundary lane at exactly `last_touch_blue_score = B.blue_score - F` is therefore still present from `B`'s POV and only becomes purged at the next block.

This boundary matters for same-block reactivation: a lane whose latest tip sits at `B.blue_score - F - 1` is *absent* from `B`'s effective pre-update state, so a touch in `B` produces a reset (`parent_ref = SeqCommit(parent(B))`) per Section 5.2 step 3.
...
where `lane_tip_hash` and `last_touch_blue_score` are taken from `ActiveLanes[lane]`.
...
`lane_id_bytes(lane)` is intentionally **not** included in the leaf payload: it is already bound twice, by the SMT key path (`lane_key(lane) = H_lane_key(lane_id_bytes(lane))`, Section 6.1) and recursively inside `lane_tip_hash` via `H_lane_tip(...)` (Section 4.3). Repeating it in the leaf payload would add bytes without changing the cryptographic binding.
...
This KIP defines `SMT_Root` as a logically fixed-depth (256) sparse Merkle tree, with single-leaf subtrees collapsed at the hashing layer (Section 6.4.1):
...
##### 6.4.1 Single-Leaf Subtree Collapsing (SLO) (Normative)

A subtree containing exactly one active leaf is hashed as a single *collapsed* node, not as the full chain of internal nodes down to depth 256. The collapsed-node hash uses a dedicated domain `H_collapsed_node`:

```
collapsed_node_hash(lane_key, leaf_hash_lane) =
    H_collapsed_node(lane_key || leaf_hash_lane)
```

`H_collapsed_node` is a domain disjoint from `H_node` and `H_leaf` (see Section 4.0), so collapsed and branch-internal preimages cannot be confused (second-preimage separation). The root of an SMT containing exactly one active lane is therefore `collapsed_node_hash(lane_key, leaf_hash_lane)`, not `EMPTY_255`-chained up from the leaf.

A proof format that compresses empty siblings with a 32-byte bitmap and records the terminal as `Full` / `Collapsed` / `CollapsedOther` is described in the companion KIP-21-RK document (Section 2.1) along with reference implementation pointers.
...
#### 6.6 Activity Root (Normative)

The active-lanes SMT root is wrapped one level above into an *activity root* that also commits a per-block `inactivity_shortcut(B)` field:

```
ActivityRoot(B) = H_activity_root( inactivity_shortcut(B) || ActiveLanesRoot(B) )
```

`inactivity_shortcut(B)` is the `SeqCommit` of the most recent non-genesis selected-parent ancestor whose blue score sits beyond the current staleness boundary, or `ZERO_HASH` when no such ancestor exists (near-genesis sentinel). It exists so L2 guest verifiers can prove "lane was silent for `N` blocks" in `O(N/F)` walked anchors instead of `O(N)`, **without baking the value of `F` into the guest**: the structural commitment encodes the parameter implicitly.

This wrap is the only change to the commitment composition introduced for the shortcut. Lane tips themselves remain shortcut-free; the SMT, lane-tip update rule (Section 4.3), and purge logic do not depend on the shortcut value.

The normative definition of `inactivity_shortcut(B)`, including the `min(F(A), F(B))` predicate that handles future `F`-rotation in either direction, the genesis exclusion, the `ZERO_HASH` near-genesis sentinel, the L1 invariants (lane membership, reset-choice, back-link binding, chain linkage), and the guest-side verification algorithm with its soundness arguments, lives in the standalone *Inactivity Shortcut Spec*: [https://github.com/kaspanet/vprogs/blob/feat/inactivity\_proofs/docs/kaspa-inactivity-shortcut-spec.md](https://github.com/kaspanet/vprogs/blob/feat/inactivity_proofs/docs/kaspa-inactivity-shortcut-spec.md). That document is normative for the field's value and for the proof format consuming it.

*Reader note on the Inactivity Shortcut Spec*. That spec is written with a separate "ZK-hardening" activation `daa_score` `H_daa` distinct from the base KIP-21 activation, reflecting an earlier two-stage rollout. On the network targeted by this KIP-21 revision the two stages collapse into a single hard fork, so `H_daa` equals the KIP-21 activation `daa_score` and every reference to "in-domain" / "pre-activation" / "activation prefix" in that spec reduces to the standard pre-fork vs post-fork distinction. The near-genesis `ZERO_HASH` sentinel and the genesis-exclusion rule retain their meaning unchanged; the activation-prefix sentinel branch is absorbed into the near-genesis case.

#### 6.7 Sequencing State Root (Normative)
...
* `ActivityRoot(B)` is a direct child of `SeqStateRoot(B)`. It is the `H_activity_root` wrap of Section 6.6.
* `MergesetContextHash(B)` and `MinerPayloadRoot(B)` are committed as the right child hash. The reference implementation names this combined right child `payload_and_ctx_digest`.

#### 6.8 Sequencing Commitment Recurrence (Normative)
...
* `SeqStateRoot(B)`, `ActivityRoot(B)`, and `ActiveLanesRoot(B)` are not directly exposed in the header; all are committed transitively by `SeqCommit(B)`.
...
This KIP defines `accepted_id_merkle_root` semantics for `OpChainblockSeqCommit` from the activating hard fork onward.
...
#### 9.1 Index Requirements (Normative)

Implementations MUST provide an on-disk index that supports, for any given pair of blue-score bounds `(lo, hi)`:

* iteration over the lanes whose `last_touch_blue_score ∈ [lo, hi]` in deterministic order, without rescanning every lane in `ActiveLanes`;
* per-chain-block retrieval of the *manifest* of lanes touched or expired by that block, sufficient to drive reverse application of the block's `SeqCommitDiff` (Section 8) without recomputation.

The concrete RocksDB-friendly layout used by the reference implementation, including key encoding, two `kind` values (`LeafUpdate` / `Structural`), batched-import keying, and the per-entry `max_depth` field, is specified in [KIP-21-RK §8.4](./kip-0021-rk.md#84-scoreindex-purge--replay-index).

#### 9.2 In-Memory Cache

An implementation may maintain in-memory caches over the lane-version and branch-version stores. Cache correctness is defined by a *newest-suffix invariant* (for every entity, the cached set is a score-ordered suffix of the entity's persisted write history) and three preserving rules (write-through on the incremental path, score-ordered eviction, canonical tie-break); see [KIP-21-RK §9.3](./kip-0021-rk.md#93-newest-suffix-cache-invariant-normative-for-cache-correctness). Caches are a performance layer only; canonical correctness is defined by persisted state.
...
* Full active-lane leaf data at PP (`lane_key`, `lane_tip_hash`, `last_touch_blue_score` for every active lane), sufficient to reconstruct `ActiveLanesRoot(PP)` in full. `lane_id_bytes` is not part of the consensus state past the SMT key derivation (the SMT and `H_lane_tip` only see `lane_key`), so it is not part of the IBD wire payload; the receiver verifies the streamed `lane_key` values indirectly by checking that the reconstructed `ActiveLanesRoot(PP)` matches the value bound into `SeqCommit(PP)`.
...
* The value of `inactivity_shortcut(PP)`. Implementations may instead persist an internal `inactivity_shortcut_block` pointer (the block hash being targeted, used as a forward-walk search seed) and fold it to the committed value via the rules in the *Inactivity Shortcut Spec* (Section 2.2); the folded value is what feeds `ActivityRoot(PP)` (Section 6.6).

These are sufficient to verify `SeqStateRoot(PP)` and then verify `PP.accepted_id_merkle_root = SeqCommit(PP)` by the recurrence in Section 6.8.
...
* SMT update is $O((t+p)\log(2^{256})) = O((t+p)\cdot 256) = O(t+p)$ worst-case. With the SLO collapsing of Section 6.4.1, the average per-update depth is well below 256 in practice (subtrees holding a single active lane skip directly to the collapsed-node hash), so the realized constant is much smaller than the worst-case 256.
...
For a lane that was purged and later re-appears, proving continuity needs both lane inclusion and lane non-inclusion witnesses anchored to the surrounding `SeqCommit` chain. The per-block `inactivity_shortcut(B)` field (Section 6.6) collapses the walked-anchor count from `O(N)` per-`F`-window non-inclusion checkpoints to `O(N/F)` anchor hops, without requiring the verifier to know the value of `F`.

The full proof format, including the dual-mode (Shortcut / Walk) anchor-to-anchor transition, the `ZERO_HASH` walk-termination semantics for the near-genesis sentinel, the reset-emission witness at the reactivation block, and the soundness and completeness arguments under both constant and dynamic `F`, is specified in the *Inactivity Shortcut Spec*, Sections 4 and 5:

* [https://github.com/kaspanet/vprogs/blob/feat/inactivity\_proofs/docs/kaspa-inactivity-shortcut-spec.md](https://github.com/kaspanet/vprogs/blob/feat/inactivity_proofs/docs/kaspa-inactivity-shortcut-spec.md)

The standalone spec is normative for the proof format consuming `inactivity_shortcut(B)`; this KIP is normative for the field's commitment placement (Section 6.6) and IBD bootstrap requirements (Section 9.4).
...
* Maintain a unified, content-addressed commitment-node store:
  ...
  covering both `H_seq` nodes (sequencing commitment, state-root, and activity-root composition) and SMT internal nodes (`H_node` and `H_collapsed_node` under `ActiveLanesRoot`).

* Witnesses can then be reconstructed directly from a header anchor `accepted_id_merkle_root = SeqCommit(B)` by descending the stored commitment DAG. The fixed prefix path from `SeqCommit(B)` to `ActiveLanesRoot(B)` is: right child to `SeqStateRoot(B)`, then left child to `ActivityRoot(B)`, then right child of `ActivityRoot(B)` to `ActiveLanesRoot(B)`. From there, follow the SMT path for `lane_key(lane)` while collecting siblings, stopping early at a collapsed-subtree terminal where applicable (Section 6.4.1).

* Empty subtree hashes (`EMPTY_i`) are computable from the spec; the store only needs to retain non-empty nodes. Collapsed nodes (Section 6.4.1) are stored as a single `(lane_key, leaf_hash)` entry rather than as a chain of 256 internal nodes.
...
[7] KIP-21-RK: Reference Design (byte-level encodings, SMT/SLO layout, IBD bootstrap, witness store): [./kip-0021-rk.md](./kip-0021-rk.md)  
[8] Inactivity Shortcut Spec (definition of `inactivity_shortcut(B)`, guest verification, soundness, witness sourcing): [https://github.com/kaspanet/vprogs/blob/feat/inactivity\_proofs/docs/kaspa-inactivity-shortcut-spec.md](https://github.com/kaspanet/vprogs/blob/feat/inactivity_proofs/docs/kaspa-inactivity-shortcut-spec.md)  
