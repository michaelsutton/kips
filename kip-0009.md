...
**Transaction validation in context** During contextual transaction validation (i.e., with full UTXO context of the containing block or a chain block merging it), the full mass calculation is verified and the transaction is ignored if the committed mass field is different than the expected mass. Like other transaction validation rules, blocks with wrongly committed mass should be considered disqualified from the selected-chain. 
