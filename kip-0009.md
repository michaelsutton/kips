...
The formal specification of the suggested changes is described below. Detailed background and analysis will be provided thereafter.

### Extended mass formula 

A new quantity is proposed, named the `storage_mass` (defined per transaction). The current notion of mass is referred to as `compute_mass`. Both quantities represent different kinds of resources, thus the overall mass formula is the maximum over both:
...
where C is a constant controlling the correlation of inverse KAS value to mass units. Below we will discuss concrete values for $C$.
...
We call this the *relaxed formula*. As shown in [3], `storage_mass*` can be used for transactions satisfying $|O|\le|I|\le 2 \lor |O|=1$.
...
def negative_mass(I, |O|):
    """
    Calculates the negative component of the storage mass formula. Note that there is no dependency on
    output values but only on their count. The code is designed to maximize precision and to avoid overflows.  
    In practice, all calculations should be saturating, i.e., bounded by u64::MIN, u64::MAX  
    """
    if |O| == 1 or |O| <= |I| <= 2:
...
def mass(tx):
    return max(storage_mass(I, O), compute_mass(tx)) 
```
...
**Block template** The new mass should be used by the mempool transaction selection algorithm to calculate the $fee/mass$ ratio, and the overall block template mass should respect the block mass limit ($500,000$ grams).

### Consensus (hardfork)
    Spec of new tx field
    Block validation rule
    UTXO validation rule

### Wallets 
Short def of micropayments and some stats indicating this is currently only relevant for faucet-like usages.

Wallet developers wishing to provide support for micropayments, have two possible routes:

    Actual example (with numbers) of a few chained txs for micropayments
    Mutual tx

### RPC API
Once the hard-fork is applied, all mining software must update their RPC interface to a version that includes the new transaction mass field. Otherwise, they would create invalid blocks.
